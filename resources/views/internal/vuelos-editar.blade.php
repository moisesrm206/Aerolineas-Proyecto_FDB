    ...
    <section class="space-y-8">
        <div class="grid gap-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
            <div>
                ...
                <h1 class="mt-3 text-4xl font-bold tracking-tight text-white sm:text-5xl">Editar vuelo</h1>
                <p class="mt-4 max-w-2xl text-base leading-7 text-white/70 sm:text-lg">
                    Ajusta ruta, aeronave, horario y estado operativo del vuelo sin salir del flujo de administración.
                </p>
            </div>
            ...
            <form action="{{ route('admin.vuelos.actualizar', $vuelo->id_vuelo) }}" method="POST" class="grid gap-6">
                ...
                <div class="grid gap-6 lg:grid-cols-2">
                    ...
                    <div class="space-y-2">
                        ...
                        <input id="salida_planificada" name="salida_planificada" type="datetime-local" value="{{ optional($vuelo->salida_planificada)->format('Y-m-d\TH:i') }}" class="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder:text-white/35 focus:border-cyan-300 focus:outline-none focus:ring-2 focus:ring-cyan-300/30">
                    </div>

                    <div class="space-y-2">
                        <label class="block text-sm font-medium text-white/80" for="llegada_planificada">Llegada planificada</label>
                        <input id="llegada_planificada" name="llegada_planificada" type="datetime-local" value="{{ optional($vuelo->llegada_planificada)->format('Y-m-d\TH:i') }}" class="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder:text-white/35 focus:border-cyan-300 focus:outline-none focus:ring-2 focus:ring-cyan-300/30">
                    </div>
